...
Node *Parser::parse_FuncDef(int depth) {
    ...
    if (this->curToken->symbol != Symbol::RPARENT &&
        this->curToken->symbol != Symbol::LBRACE) {
        // 要求也不等于 {， 否则可能是错误处理中的 int func2({
        // FuncFParams
...
Node *Parser::parse_MainFuncDef(int depth) {
    ...
    if (this->curToken->symbol == Symbol::RPARENT) {
        mainFuncDef->addChild(new Node(this->curToken, mainFuncDef, depth + 1));
        this->nextItem();
    } else {  // FIXME: 错误 j => ErrorType::MissingRPARENT )
        int formerLineNum = this->tokens[tokenPos - 1]->lineNumber;
        mainFuncDef->addChild(new ErrorNode(ErrorType::MissingRPARENT, formerLineNum));
    }
    // Block
    child = this->parse_Block(depth + 1);
...
Node *Parser::parse_ConstDecl(int depth) {
    ...
    if (this->curToken->symbol == Symbol::SEMICN) {
        constDecl->addChild(new Node(this->curToken, constDecl, depth + 1));
        this->nextItem();
    } else {  // FIXME: 错误i => ErrorType::MissingSEMICN
        int formerLineNum = this->tokens[tokenPos - 1]->lineNumber;
        constDecl->addChild(new ErrorNode(ErrorType::MissingSEMICN, formerLineNum));
    }
    return constDecl;
}
...
Node *Parser::parse_VarDecl(int depth) {
    ...
    if (this->curToken->symbol == Symbol::SEMICN) {
        ...
        int formerLineNum = this->tokens[tokenPos - 1]->lineNumber;
        varDecl->addChild(new ErrorNode(ErrorType::MissingSEMICN, formerLineNum));
    }
    return varDecl;
}
...
#ifdef PARSER_DEBUG
    std::cout << "\nParsing BType:\nline: " <<
              this->curToken->lineNumber << ", value: " <<
              this->curToken->value << std::endl;
#endif
    assert(this->curToken->symbol == Symbol::INTTK);
...
Node *Parser::parse_ConstDef(int depth) {
    ...
    while (this->curToken->symbol == Symbol::LBRACK) {
        ...
        constDef->addChild(child);
        // ']'
        if (this->curToken->symbol == Symbol::RBRACK) {
            constDef->addChild(new Node(this->curToken, constDef, depth + 1));
            this->nextItem();
        } else {  // FIXME: k => ErrorType::MissingRBRACK
            constDef->addChild(new ErrorNode(ErrorType::MissingRBRACK,
                                             this->tokens[tokenPos - 1]->lineNumber));
        }
    }
    // '='
...
Node *Parser::parse_VarDef(int depth) {
    ...
    while (this->curToken->symbol == Symbol::LBRACK) {
        ...
        if (this->curToken->symbol == Symbol::RBRACK) {
            ...
            varDef->addChild(new ErrorNode(ErrorType::MissingRBRACK,
                                           this->tokens[tokenPos - 1]->lineNumber));
        }
    }
...
Node *Parser::parse_FuncFParam(int depth) {
    ...
    if (this->curToken->symbol == Symbol::LBRACK) {
        ...
        if (this->curToken->symbol == Symbol::RBRACK) {
            ...
            funcFParam->addChild(new ErrorNode(ErrorType::MissingRBRACK,
                                               this->tokens[tokenPos - 1]->lineNumber));
        }
        while (this->curToken->symbol == Symbol::LBRACK) {
            funcFParam->addChild(new Node(this->curToken, funcFParam, depth + 1));
...
/*
 * Stmt → LVal '=' Exp ';' FIXME: i => ErrorType::MissingSEMICN
 ...
 *      | 'break' ';' FIXME: i => ErrorType::MissingSEMICN
 *      | 'continue' ';' FIXME: i => ErrorType::MissingSEMICN
 *      | 'return' [Exp] ';' FIXME:i => ErrorType::MissingSEMICN
 *      | LVal '=' 'getint''('')'';'  FIXME: i => ErrorType::MissingSEMICN;
 *                                    FIXME: j => ErrorType::MissingRPARENT )
...
Node *Parser::parse_Stmt(int depth) {
    ...
    if (this->curToken->symbol == Symbol::PRINTFTK) {  /* 'printf''('FormatString{','Exp}')'';' */
        ...
        if (this->curToken->symbol == Symbol::SEMICN) {
            ...
            int formerLineNum = this->tokens[tokenPos - 1]->lineNumber;
            stmt->addChild(new ErrorNode(ErrorType::MissingSEMICN, formerLineNum));
        }
        ...
        // [Exp]
        if (this->curToken->symbol != Symbol::SEMICN && (
                this->curToken->symbol == Symbol::LPARENT ||
                this->curToken->symbol == Symbol::IDENFR ||
                this->curToken->symbol == Symbol::INTCON ||
                this->curToken->symbol == Symbol::PLUS ||
        ...
        child->setParent(stmt);
        stmt->addChild(child);
    } else if (!this->hasAssignInCurrentLine()) {  /* 接下里本行没有等号 [Exp] ';' */
        if (this->curToken->symbol != Symbol::SEMICN) {
            child = this->parse_Exp(depth + 1);
            child->setParent(stmt);
            stmt->addChild(child);
...
bool Parser::hasAssignInCurrentLine() {
    /* 判断接下来在当前行是否有 '=' */
    /* 用于Stmt解析中判断是表达式还是赋值 */
    auto i = this->tokenPos, curLineNum = curToken->lineNumber;
    while (this->tokens[i]->symbol != Symbol::SEMICN &&
           this->tokens[i]->symbol != Symbol::ASSIGN &&
           this->tokens[i]->lineNumber == curLineNum) {
        i++;
        if (i >= tokens.size()) return false;
    }
    if (this->tokens[i]->symbol == Symbol::ASSIGN) return true;
    return false;
}
...
Node *Parser::parse_LVal(int depth) {
    ...
    while (this->curToken->symbol == Symbol::LBRACK) {
        ...
        if (this->curToken->symbol == Symbol::RBRACK) {
            ...
            lVal->addChild(new ErrorNode(ErrorType::MissingRBRACK,
                                         this->tokens[tokenPos - 1]->lineNumber));
        }
    }
    return lVal;
...
Node *Parser::parse_UnaryExp(int depth) {
    ...
    if (this->curToken->symbol == Symbol::PLUS
        ...
        this->nextItem();
        if (this->curToken->symbol != Symbol::RPARENT &&
            this->curToken->symbol != Symbol::SEMICN) {  // [FuncRParams]
        ...
        if (this->curToken->symbol == Symbol::RPARENT) {
            ...
            unaryExp->addChild(new ErrorNode(ErrorType::MissingRPARENT,
                                             this->tokens[tokenPos - 1]->lineNumber));
        }
    } else {  // PrimaryExp
